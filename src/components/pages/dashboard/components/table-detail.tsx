  ...
  return (
    <div className="flex flex-col w-full rounded-md overflow-auto overflow-x-auto shadow-md">
      <div className="grid grid-cols-6 text-center">
        ...
        <span className="py-1 bg-blue-400 text-white">
          DAYA MAMPU NETTO (MW)
        </span>
        <span className="py-1 bg-blue-400 text-white">
          DAYA MAMPU PASOK (MW)
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="item-1">
          <AccordionTrigger className="grid grid-cols-6 font-bold">
            <span className="py-1 col-span-2 bg-blue-50 text-gray-700 text-left px-4">
              BMPP
            </span>
            <span className="py-1 bg-blue-50 text-gray-700">
          ...
          <AccordionContent>
            <div className="grid grid-cols-6 mt-1 text-center bg-gray-50">
              {data?.["BMPP WAAI"]?.detail?.map((obj, idx) => {
                return (
                  <Fragment key={idx}>
                    <span className="py-1 col-span-2 text-left px-8 border-b">
                      {obj.unit_subname}
                    </span>
                    <span className="py-1 border-b">
                      {roundNumber(obj.p_dmp_netto)}
                    </span>
                    <span className="py-1 border-b">
                      {roundNumber(obj.p_dmp_pasok)}
                    </span>
                    <span className="py-1 border-b">{roundNumber(obj.p)}</span>
                    <span className="py-1 border-b">{roundNumber(obj.v)}</span>
                  </Fragment>
                );
              })}
            </div>
