...
export default function CloudRender({ name }: { name: string }) {
  return (
    <>
      ...
      {name === "Rain" && <CloudRain className="h-16 w-16" />}
      {name === "Clouds" && <Cloudy className="h-16 w-16" />}
      {name === "Drizzle" && <CloudDrizzle className="h-16 w-16" />}
      {name === "Thunderstorm" && <CloudLightning className="h-16 w-16" />}

      {name !== "Thunderstorm" &&
        name !== "Clear" &&
        name !== "Rain" &&
        name !== "Clouds" &&
        name !== "Drizzle" && <Cloud className="h-16 w-16" />}
    </>
  );
}
